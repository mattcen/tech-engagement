define(`talk_author', `Matthew Cengia')
define(`author_twitter', `mattcen')
define(`github_user', `mattcen')
define(`github_repo', `tech-engagement')
define(`talk_title', `Preparing to be makers in a consumer-focussed world')
define(`talk_description', `Teaching newbies to hack & develop')
define(`talk_tagline', `Teaching newbies to hack & develop')
define(`talk_hashtag', `#techengagement')
define(`talk_whoami', `  <li>Linux Systems Adminstrator/Developer</li>
          <li>Scout Leader</li>
          <li>Mapping Hobbyist</li>
          <li>Open source, privacy, security, and diversity advocate, and <a href="http://au.okfn.org">Open Knowledge Australia ambassador (au.okfn.org)</a></li>
')
